...
def parse_args():
    ...
    parser.add_argument(
        "--cfg",
        type=str,
        default="/home/user/PROJECT/pp/NEU-DET_YOLO_state_qmh/NEU-DET.yaml",
    ...
    parser.add_argument("--epochs", type=int, default=300, help="训练 epochs (默认: 300)")
    parser.add_argument("--batch-size", type=int, default=16, help="batch size (默认: 16)")
    parser.add_argument("--img-size", type=int, default=640, help="输入图片大小 (px) (默认: 640)")
    parser.add_argument(
        "--optimizer", type=str, default="Adam", choices=["SGD", "Adam"], help="优化器类型 (SGD 或 Adam) (默认: Adam)"
    )
    # 降低初始学习率
    parser.add_argument("--lr0", type=float, default=0.001, help="初始学习率(默认:0.001)")
    parser.add_argument("--weight-decay", type=float, default=0.0005, help="权重衰减 (weight decay) (默认: 0.0005)")
    parser.add_argument("--momentum", type=float, default=0.937, help="优化器动量 (momentum) (默认: 0.937)")
    parser.add_argument("--project", type=str, default="runs/train", help="实验结果保存目录 (默认: runs/train)")
    parser.add_argument(
        "--name",
        type=str,
        default="baseline_yolo11",
...
if __name__ == "__main__":
    ...
    with open(custom_yaml, encoding="utf-8") as f:
        yaml_content = f.read()
    print("YAML 文件正文如下：\n" + yaml_content)
